package com.spharos6th.authservice.adaptor.out.security.provider;
...
public class JwtTokenProvider {
     ...
     */
    public String validateAndGetUserUuid(String token) throws Exception {
        try {
            log.info(extractClaim(token, Claims::getSubject));
            return null;
        } catch (NullPointerException e) {
            log.info("토큰에 담긴 유저 정보가 없습니다");
            throw new Exception("토큰에 담긴 유저 정보가 없습니다");
        }
    }
    ...
    public String generateAccessToken() {
        Date now = new Date();
        Date expiration = new Date(now.getTime() + Objects.requireNonNull(
                env.getProperty("JWT.token.access-expire-time", Long.class),
                "JWT token access-expire-time property is missing"));
        return Jwts.builder()
                .signWith(getSignKey())
                .issuedAt(expiration)
                .compact();
